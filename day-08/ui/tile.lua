...
function Tile:draw(grid, x, y)
    ...
    local baseScale = grid.tileWidth / (imgW)
    local scale = baseScale * grid.zoom
    local scaledW = (imgW) * scale
    local scaledH = (imgH) * scale
    local drawX = isoX + (grid.tileWidth * grid.zoom) / scaledW / 2
    local drawY = isoY + (grid.tileHeight * grid.zoom) - scaledH
